...
- **Screen-time passcode box is typeable again** — TikTok's "Ready to close TikTok?" dialog inherits `user-select: none` from the app's captcha-slider CSS (its container carries a `verify`-family class), which made the input unable to accept typed text. Form controls now always keep native text selection/typing, the captcha selectors are scoped to never match form controls, and the carousel click-suppression never engages on a text field — so no other dialog, search box, or composer can be affected the same way.
