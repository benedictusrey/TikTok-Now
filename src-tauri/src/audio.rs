//! Windows audio-session control for TikTok-Now.
//!
...
//! (e.g. the webview is minimized and `eval` is throttled, or TikTok's player
...
pub fn report_audio_state() {
    unsafe {
        if CoInitializeEx(None, COINIT_MULTITHREADED).0 < 0 {
            eprintln!("[TikTok-Now] Audio state: COM init failed");
            return;
        }
        let Ok(enumerator) =
            CoCreateInstance::<_, IMMDeviceEnumerator>(&MMDeviceEnumerator, None, CLSCTX_ALL)
        else {
            eprintln!("[TikTok-Now] Audio state: no audio endpoint");
            CoUninitialize();
            return;
        };
        let Ok(device) = enumerator.GetDefaultAudioEndpoint(eRender, eMultimedia) else {
            eprintln!("[TikTok-Now] Audio state: no default render device");
            CoUninitialize();
            return;
        };
        let Ok(manager) = device.Activate::<IAudioSessionManager2>(CLSCTX_ALL, None) else {
            eprintln!("[TikTok-Now] Audio state: no session manager");
            CoUninitialize();
            return;
        };
        let Ok(sessions) = manager.GetSessionEnumerator() else {
            eprintln!("[TikTok-Now] Audio state: no session enumerator");
            CoUninitialize();
            return;
        };
        let Ok(count) = sessions.GetCount() else {
            eprintln!("[TikTok-Now] Audio state: no session count");
            CoUninitialize();
            return;
        };
        ...
        if found == 0 {
            eprintln!("[TikTok-Now] Audio state: no sessions in the process tree");
        }
        CoUninitialize();
    }
}
